    ...
    async def command_action(self, client, message, _):
        ...
        if 'action' in message:
            if message['action'] == 'alarm':
                ...
                await self.hbf.publish('/haspa/licht/alarm', 1)
                await asyncio.sleep(2)
                await self.hbf.publish('/haspa/licht/alarm', 0)

            elif message['action'] == 'strobo':
                for i in range(100):
                    await self.hbf.publish('/haspa/licht/c', 0)
                    await asyncio.sleep(0.05)
                    await self.hbf.publish('/haspa/licht/c', 1023)
                    await asyncio.sleep(0.03)

            elif message['action'] == 'party':
                await self.hbf.publish('/haspa/licht', 0)
                await self.hbf.publish('/haspa/licht/c', 0)
                await self.hbf.publish('/haspa/licht/w', 0)
                delay = 0.05
                ...
                if sound[0] == '97':
                    ...
                    for i in range(0, 300):
                        ...
                        await self.hbf.publish('/haspa/licht/w', i * 10/3)
                    ...
                    for i in range(2):
                        ...
                        await self.hbf.publish('/haspa/licht/1/c', 1023)
                        await self.hbf.publish('/haspa/licht/1/c', 1023)
                        await self.hbf.publish('/haspa/licht/alarm', 1)
                        ...
                        for o in range(40):
                            await self.hbf.publish('/haspa/licht/c', 20 * o)
                            await asyncio.sleep(0.01)
                        ...
                        for o in range(20):
                            await self.hbf.publish('/haspa/licht/1/c', 1023)
                            await self.hbf.publish('/haspa/licht/3/c', 0)

                            await asyncio.sleep(0.01)

                            await self.hbf.publish('/haspa/licht/1/c', 0)
                    ...
                    for _ in range(int(sound[1]/(delay * 4))):
                        ...
                        await self.hbf.publish('/haspa/licht/1/c', 0)
                        await self.hbf.publish('/haspa/licht/4/c', 1023)
                        await asyncio.sleep(delay)
